...
impl TorrentStateLive {
    ...
    async fn task_manage_incoming_peer(
        ...
        let options = PeerConnectionOptions {
            ...
            client_version: self.shared.options.peer_client_version,
            ..Default::default()
        };
    ...
    async fn task_manage_outgoing_peer(
        ...
        let options = PeerConnectionOptions {
            ...
            client_version: state.shared.options.peer_client_version,
            ..Default::default()
        };
        let peer_connection = PeerConnection::new(
            addr,
    ...
    async fn task_peer_adder(
        ...
        let state = self;
        loop {
            let (handle, addr) = peer_queue_rx.recv().await.context("torrent closed")?;
    ...
    pub(crate) fn add_peer_if_not_seen(&self, addr: SocketAddr) -> anyhow::Result<bool> {
        ...
        self.peer_queue_tx.send((addr, addr))?;
        Ok(true)
    }
            ...
            self.state.clone().spawn(
                ...
                async move {
                    ...
                    let outgoing_addr = self
                        .state
                        .peers
                        .with_peer_mut(handle, "dead_to_queued", |peer| match peer.get_state() {
                            PeerState::Dead => {
                                peer.set_state(PeerState::Queued, &self.state.peers);
                                Ok(peer.outgoing_address.unwrap_or(handle))
                            }
                            other => bail!(
                    ...
                    self.state.peer_queue_tx.send((handle, outgoing_addr))?;
                    Ok::<_, anyhow::Error>(())
                },
            );
        } else {
