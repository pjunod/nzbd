...
- ✅ Settings tab: `GET/PUT /api/v1/config` — the live TOML with secrets masked (`***unchanged***` round-trips without retyping passwords), strict validation on save, write + hot-reload via the generalized setup/reload handle (always present now); UI editor with save/discard/download, read-only fallbacks for unwritable paths and `--config`-less runs; fixed a latent bug where a configured speed limit was ignored whenever a queue snapshot existed (config now wins when set)
