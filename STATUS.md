...
- ✅ Handoff visibility (field feedback: "the *arr handoff is a black box"): the pull is now observed — history entries record first/last-seen (client polls), seen-count, and imported-by (NZBGet HistoryDelete = hide + stamp, matching real *arr behavior; FinalDelete = remove); native history includes hidden rows + handoff fields; `GET /api/v1/clients` + UI strip show every compat consumer (UA, polling/quiet, cadence); History tab handoff chips (awaiting pickup / seen ×N / imported by) with per-entry restore (re-expose for re-import), hide, forget, delete-files; hidden state survives index rebuilds via JSONL re-append upsert
- ✅ UI boot smoke test: node harness executes the embedded page's inline JS against a DOM shim on every `cargo test` — fails on load-time errors, unhandled rejections, `$()` lookups of missing ids, or boot never starting the SSE plumbing (the class of bug where the script parses but dies at load; proven against the real connectSse regression). Self-skips without node; NZBD_REQUIRE_TOOLS makes CI strict
