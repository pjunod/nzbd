...
**Snapshot (2026-07-25):** 221 tests · clippy clean · **phases 0–4
...
- ⬜ **UI v2 M0**: redeploy nuc3 on a current image, adding the missing `./config:/etc/nzbd` bind first (`mkdir config && docker cp nzbd:/etc/nzbd/nzbd.toml ./config/ && chown 1000 config/nzbd.toml`, add the mount, `docker compose up -d --build`) — without it the wizard config dies on every rebuild, and the deployed image still predates the `no-store` cache fix
