...
| 6. Path and delete safety | **Pass** | The adapter now repeats the portable lexical path invariant before rqbit admission: empty/dot/parent components, slash or backslash, absolute/UNC forms, Windows drive prefixes, NUL, malformed UTF-8, empty multi-file paths, missing/unsafe multi-file roots, and metainfo-declared symlinks fail with a safe nzbd-owned error. A real-admission corpus still proves rejection before an escape file exists. Delete-data removes only parsed torrent content; an unrelated sibling survives. Higher layers must still prove the persisted canonical root before deletion; existing-filesystem symlink and normalized case-collision checks remain M5 work. |
