...
**One self-contained page, no build step.** `crates/nzbd-api/ui/index.html` is compiled into the binary with `include_str!` and served at `/`; the PWA manifest, service worker and icons ride along as `include_bytes!`. The originally planned Svelte 5 + Vite SPA was dropped in phase 4 and the decision re-confirmed in the v2 rebuild (2026-07-25): a Node build chain in CI is a real and permanent cost, and the two things it would have bought — component structure and a diffing runtime — turned out to be ~150 lines of keyed reconciler for one table plus a handful of tiles. Vendoring preact+htm was rejected on the same arithmetic. This is a deliberate simplification and it has limits: past roughly 120 KB the single file should split into several embedded assets, and past ~500 queue rows the table wants virtualizing (nzbget's `fasttable` lesson — large queues must not freeze the UI). Neither threshold is close today.
