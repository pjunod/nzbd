...
| 19 | **BitTorrent: one protocol-neutral queue/backend seam; production engine still gated; immutable seed tree; readiness is durable state** ([proposal](BITTORRENT_PROPOSAL.md), [M0 report](BITTORRENT_M0_REPORT.md), [M1b report](BITTORRENT_M1B_REPORT.md)). **Amended:** the 8.1.1 spike and an unreleased-9.0 re-check both fail authoritative fast-resume and discovery-observability gates, so M2 remains blocked. M1b is engine-independent and dormant: schema 3, fake mixed scheduling, reliable structural facts, and coalesced progress may land without daemon networking. Proxy use remains fail-closed for DHT and UDP trackers. | A second daemon (two queue authorities and lifecycle stores); pretending pieces/peers are NNTP articles/servers (dishonest state); pinning a release candidate or entering `libtorrent` FFI before an explicit engine decision. First release is deliberately v1 TCP/IPv4 and single-node. |
