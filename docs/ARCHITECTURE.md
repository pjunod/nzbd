...
**Resume protocol.** The engine's `broadcast` channel is lossy by design — a slow subscriber is dropped rather than allowed to stall the queue owner — which is right for a browser tab and wrong for a consumer that imports files. Every engine frame therefore carries `id: <seq>`, a monotone `u64` also embedded in the body as `"seq"`, stamped by a single pump task in `nzbd-api::eventhub` so that all consumers see the same numbers in the same order. The last 1024 frames are kept; a client reconnecting with `Last-Event-ID` gets exactly the missed tail replayed before live frames resume, and a gap the ring cannot cover — or an id from a previous process, since `seq` is process-lifetime and resets on restart — answers `event: reset {"reason":"gap"}` instead of a stream that looks contiguous and is not. `tick`, `hb` and `log` carry no `id:`: they are the stream's own views, not engine facts, and resuming from one would mean nothing.
