...
**Liveness.** Connection state is `live` · `reconnecting` · `unreachable`, the last raising a page-wide banner rather than a gray dot — after two consecutive failed polls, since one alone is a race with a restart. The rate tile carries a 180-sample sparkline (a number says what is happening now; the shape says whether that is normal), the tab title tickers `▼ 93 MiB/s · 12m — nzbd` and resets when idle, per-provider chips show each server's share of the wire rate and go red when it is blocked, and the Logs tab is a `tail -f` over the `log` stream with a 500-line client ring that marks any gap the server's batch cap created.
