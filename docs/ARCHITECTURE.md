...
| 19 | **BitTorrent: one protocol-neutral queue/backend seam; production engine still gated; immutable seed tree; readiness is durable state** ([proposal](BITTORRENT_PROPOSAL.md), [M0 report](BITTORRENT_M0_REPORT.md), [M1b report](BITTORRENT_M1B_REPORT.md)). **Amended:** the maintained nine-patch rqbit v8.1.1 engine passes all eleven M0 gates and independent review. M1b landed schema 3, fake mixed scheduling, reliable structural facts, and coalesced progress without daemon networking. M2a added dormant `[torrent]` configuration and a named fail-closed activation guard; M2b added dormant selective-session and queue-adjacent runtime ownership while preserving production daemon isolation. Remaining M2 slices are tracked by #154–#160, and #163 retains sole activation ownership. No production daemon torrent session, listener, or admission path is wired. M5/M6 still require their own gates, and proxy use remains fail-closed for DHT and UDP trackers. | A second daemon (two queue authorities and lifecycle stores); pretending pieces/peers are NNTP articles/servers (dishonest state); pinning a release candidate or entering `libtorrent` FFI before an explicit engine decision. First release is deliberately v1 TCP/IPv4 and single-node. |
